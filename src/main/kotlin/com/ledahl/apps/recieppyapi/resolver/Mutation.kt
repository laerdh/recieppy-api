...
class Mutation(@Autowired private val recipeService: RecipeService,
    ...
    fun sendInvite(locationId: Long, email: String, env: DataFetchingEnvironment): Location? {
        val user = env.getContext<AuthContext>().user
    ...
    fun revokeInvite(locationId: Long, email: String, env: DataFetchingEnvironment): Location? {
        val user = env.getContext<AuthContext>().user
